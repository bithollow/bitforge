...
sudo losetup /dev/$LOOP_DEV rpi.img -o $((206848*512))
...
#'
sudo sh -c 'cat > mnt/firmware/cmdline.txt << EOF
dwc_otg.fiq_enable=0 dwc_otg.fiq_fsm_enable=0 dwc_otg.nak_holdoff=0 dwc_otg.lpm_enable=0 root=/dev/mmcblk0p2 rootfstype=ext4 elevator=deadline rootwait
EOF
'
